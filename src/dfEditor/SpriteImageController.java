    ...
    private void addSpriteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addSpriteButtonActionPerformed
        ...
        if (selectedFiles != null)
        {
            ...
            for (int i=0; i<selectedFiles.length; ++i)
            {
                ...
                {
                    BufferedImage bufferedImage = ImageIO.read(selectedFiles[i]);
                    SpriteGraphic graphic = new SpriteGraphic(bufferedImage, new java.awt.Point(0, 0), new Rectangle(0, 0, bufferedImage.getWidth(), bufferedImage.getHeight()), selectedFiles[i].getName().replace(".png", ""));
                    graphicList.add(graphic);
                }
                catch (IOException e)
                {
